  ...
  try {
    const firstRounds = await Promise.all(debated.map(async (item) => {
      ...
      const [bull, bear, valuation] = await Promise.all([
        run(env, advocatePrompt('BULL'), input, ARGUMENT_SCHEMA, 1100),
        run(env, advocatePrompt('BEAR'), input, ARGUMENT_SCHEMA, 1100),
        run(env, valuationPrompt(), input, VALUATION_SCHEMA, 900),
      ]);
      ...
      const [bullRebuttal, bearRebuttal] = await Promise.all([
        run(env, rebuttalPrompt('BULL'), JSON.stringify({ evidence: item.evidence, valuation: checkedValuation, own: checkedBull, opponent: checkedBear }), REBUTTAL_SCHEMA, 800),
        run(env, rebuttalPrompt('BEAR'), JSON.stringify({ evidence: item.evidence, valuation: checkedValuation, own: checkedBear, opponent: checkedBull }), REBUTTAL_SCHEMA, 800),
      ]);
    ...
    const [philosophyReview, riskReview] = await Promise.all([
      run(env, philosophyPrompt(), committeeInput, REVIEW_SCHEMA, 800),
      run(env, riskPrompt(), committeeInput, REVIEW_SCHEMA, 800),
    ]);
    ...
    const judged = await run(env, judgePrompt(), judgeInput, VERDICT_SCHEMA, 1500);
...
async function run(env, prompt, content, schema, maxTokens) {
  const result = await env.AI.run(MODEL, { messages: [{ role: 'system', content: prompt }, { role: 'user', content }], response_format: { type: 'json_schema', json_schema: schema }, temperature: 0, max_tokens: maxTokens });
  let value = result?.response ?? result;
  if (typeof value === 'string') value = JSON.parse(value.replace(/^```json\s*|\s*```$/g, ''));
  if (!value || typeof value !== 'object') throw new Error('Committee agent returned invalid JSON');
  return value;
}
